    ...
    pictureMessages() {
        ...
        if(cover) {
            cover.addEventListener("change", () => {
                let covDiv = cover.nextElementSibling;
                let sizeDiv = cover.nextElementSibling.nextElementSibling;
                covDiv.textContent = "";
        ...
        if(pictures) {
            pictures.addEventListener("change", () => {
                let picDiv = pictures.nextElementSibling;
                let sizeDiv = pictures.nextElementSibling.nextElementSibling;
        ...
        for(let i = 0; i < newPicture.length; i ++) {
            if(newPicture[i]) {
                newPicture[i].addEventListener("change", () => {
                    let newPic = newPicture[i].nextElementSibling;
                    let sizeDiv = newPicture[i].nextElementSibling.nextElementSibling;
                    let totalSizeDiv = newPicture[i].nextElementSibling.nextElementSibling.nextElementSibling;
                    ...
                    if(newPic && sizeDiv && totalSizeDiv) {
                        if(newPicture[i].files.length > 0) {
                            ...
                            const newPicString = `${(newPicSize / (1024 ** convNewPicSize)).toFixed(2)} ${unit[convNewPicSize]}`;
                            const trueNewPicString = newPicString.replace(".", ",");
                            ...
                            currentPicString = `${(totalPicSize / (1024 ** convertedSize)).toFixed(2)} ${unit[convertedSize]}`;
        ...
        if(extraPictures) {
            extraPictures.addEventListener("change", () => {
                let extraPicdiv = extraPictures.nextElementSibling;
                let totalSizeDiv = extraPictures.nextElementSibling.nextElementSibling;
                ...
                if(extraPicdiv && totalSizeDiv) {
                    if(extraPictures.files.length > 0) {
                        for(let i = 0; i < extraPictures.files.length; i ++) {
                            ...
                            extraPicString = `${(totalPicSize / (1024 ** convertedSize)).toFixed(2)} ${unit[convertedSize]}`;
